...
## INCLUDE FUNCTIA PENTRU CROSSOVER SINGULAR
...
# crossover singular - aplicat pe 2 permutari, date de tip lista
def crossover_sing(x,y,alpha):
    n=len(x)
    x1=x.copy()
    y1=y.copy()
    poz=np.random.randint(0,n)
    x1[poz]=alpha*x[poz]+(1-alpha)*y[poz]
    y1[poz]=(1-alpha)*x[poz]+alpha*y[poz]
    return x1,y1
...
def crossover_populatie(pop,cal,c,v,max,pc,alpha):
    ...
    for i in range(0,dim-1,2):
        ...
        if r<=pc:
            ...
            c1,c2 = crossover_sing(x,y,alpha)
...
if __name__=="__main__":
    ...
    po,co=crossover_populatie(pop,val,c,v,max,0.8,alpha)
